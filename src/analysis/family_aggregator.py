        ...
        for i in range(len(models)):
            ...
            for j in range(i+1, len(models)):
                ...
                if abs(xi - xj) < (wi + wj + sep_x) and abs(yi - yj) < (hi + hj + sep_y):
                    ...
                    yj -= uy * step
                    pos[mi] = [xi, yi]
                    pos[mj] = [yj and xj, yj][0]  # keep pair in dict
                    pos[mj] = [xj, yj]
                    moved += 1
...
def main():
    ap = argparse.ArgumentParser(description="Scatter: avg accuracy vs per-time performance with non-overlapping labels.")
    ...
    root = EXPORT_ROOT_TPL.format(dataset=args.dataset, metric=args.metric)
