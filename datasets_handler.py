...
def merge_train_or_test(folders, output_folder, train=True):
    """
    Merge only train set or only test set from given folders.

    Args:
        folders (list): List of folders to merge.
        output_folder (str): Destination folder.
        train (bool): If True, merge train data; else merge test data.
    """
    os.makedirs(output_folder, exist_ok=True)

    if train:
        files_to_merge = [
            ("train-images-idx3-ubyte", 16, ">IIII"),
            ("train-labels-idx1-ubyte", 8, ">II")
        ]
    else:
        files_to_merge = [
            ("t10k-images-idx3-ubyte", 16, ">IIII"),
            ("t10k-labels-idx1-ubyte", 8, ">II")
        ]

    for filename, header_size, header_fmt in files_to_merge:
        merged_body = b""
        total_samples = 0
        header_data = None
